  ...
  describe "when the atomino:toggle event is triggered", ->
    it "hides and shows the modal panel", ->
      ...
      runs ->
        ...
        atominoElement = workspaceElement.querySelector('.atomino')
        expect(atominoElement).toExist()

        atominoPanel = atom.workspace.panelForItem(atominoElement)
...
describe "when the atomino:compile event is triggered", ->
  it "compiles project", ->
    expect(workspaceElement.querySelector('.atomino')).not.toExist()
    atom.commands.dispatch workspaceElement, 'atomino:compile'
    waitsForPromise ->
      activationPromise
    runs ->
      expect(workspaceElement.querySelector('.atomino')).toExist()
      atominoElement = workspaceElement.querySelector('.compile')
      expect(atominoElement).toExist()
      atominoPanel = atom.workspace.panelForItem(atominoElement)
      expect(atominoPanel.isVisible()).toBe true
      atom.commands.dispatch workspaceElement, 'atomino:compile'
